...
3. 분석 내용은 문학적이고 방대하게 서술하되, 철저히 "전문적인 역학 시스템, 심층 심리학, 통계적 궤적, 에너지 상호작용"과 같은 정론적이고 분석적인 용어만을 사용하십시오. (주의: 파이썬이나 자바스크립트 같은 '실제 코루틴이나 코드 블록'은 절대 출력하지 마십시오. 은유만 사용할 뿐, 진짜 코딩을 하라는 뜻이 아닙니다.)
...
[4단계: Debugging Protocol] 발견된 취약점을 방어하고 본래의 균형을 되찾기 위한 구체적인 인지적/심리적 해결책 제시.
[5단계: Decision Making Process] 수비학의 본질수와 휴먼디자인의 '내부 권위'를 동기화하여 인생의 중요한 순간마다 오차 없이 선택을 내리는 명확한 의사결정 프로세스 도출.
...
* 각 단계별로 최소 500자 이상, 10단계를 합쳐 수천 자에 달하는 압도적인 분량과 논리로 답변하십시오. 대충 요약하면 시스템이 붕괴됩니다.
* 단, 내용은 전문적이고 깊이 있되 **말투는 절대 딱딱하거나 기계적이지 않게 하십시오**. 내담자의 마음을 깊이 이해하고 따뜻하게 위로하며 부드럽게 멘토링해 주는 '친절한 상담가'의 다정한 어투(~해요, ~합니다, ~군요 등)를 사용해 주십시오.`.trim();
    } catch (e) {
