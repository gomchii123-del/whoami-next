...
[작성 가이드라인 - 매우 중요]
1. 당신이 가진 천체력(Ephemeris) 데이터와 지식을 기반으로, 위 생년월일시와 출생지를 바탕으로 내담자의 정확한 **[휴먼디자인 바디그래프(타입, 전략, 권위, 프로필, 주요 센터의 활성화 여부)]**를 먼저 계산하고 확정하십시오. (계산에 오차가 없도록 교차 검증해 주십시오).
2. 단순한 정보 나열을 절대 금지합니다. '휴먼디자인의 메커니즘'과 '수비학의 영혼적 진동수'가 어떻게 결합되어 고유한 에너지 화학 반응을 일으키는지 유기적인 통합 분석(Synthesis)을 서술하십시오.
3. 각 단계별로 최소 300~500자 이상의 매우 상세한 서술을 유지하여, 읽는 이가 전율을 느낄 정도의 깊이를 보여주어야 합니다.
