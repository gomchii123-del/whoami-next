...
위 3가지 서로 다른 예측 프레임워크(명리학, 자미두수, 서양 점성술) 데이터를 종합하여, 다음 10가지 단계로 아주 깊이 있고 입체적인 통찰력을 제공해 주십시오:
...
[3단계: 강점의 삼각 동맹] 세 가지 관점에서 바라본 내담자의 절대적 강점, 그리고 이를 무기화할 수 있는 구체적인 방법은 무엇인가?
[4단계: 맹점과 카르마] 사주의 기구신/조후 결핍, 점성술의 하드 애스펙트(흉각), 자미두수 화기(化忌)가 가리키는 내담자의 약점과 생애 걸림돌은 무엇인가?
[5단계: 부의 철학 (재물운)] 재백궁, 점성술의 2/8하우스, 명리학적 재성/식상 구조를 통해 어느 시점에 어떤 방식으로 부를 축적해야 하는지 구체적인 투자/직업 전략을 제시하라.
[6단계: 인간관계와 인연의 법칙] 부처궁, 7하우스, 배우자성(관성/재성)을 통해 어떤 인연이 이로울지, 그리고 관계를 유지하기 위한 처세술을 서술하라.
[7단계: 커리어와 소명] 관록궁, 10하우스(MC), 당령과 팔품 임무를 바탕으로 내담자에게 최적화된 직업적 소명과 사회적 위치를 제시하라.
[8단계: 대운과 유년의 시간표] 명리학의 대운 흐름을 기반으로, 현재 내담자가 위치한 인생의 계절은 어느 때이며, 지금 당장 취해야 할 공수(공격과 수비) 전략을 분석하라.
[9단계: 개운법과 에너지 마스터리] 조후 조절용 행운의 원소, 수호 별자리 행성의 활용 등 상황을 압도적으로 반전시킬 수 있는 현실적인 개운법을 제안하라.
[10단계: 최종 예언과 영적 나침반] 모든 결론을 갈무리하여, 내담자가 자신의 천명을 긍정하고 주도적으로 삶을 개척할 수 있도록 영감이 가득 찬 최종 조언을 남겨라.

전문적인 술수를 사용해 논리적으로 전개하되, 마치 현자(Wise Guide)가 내담자의 눈을 맞추고 설명하듯 따뜻하고 감동적인 구어체로 작성하십시오.`.trim();
