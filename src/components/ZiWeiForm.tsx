...
export default function ZiWeiForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            sessionStorage.setItem('arche_report_type', 'ziwei');
            ...
            sessionStorage.setItem('lastAnalysis', JSON.stringify(ziweiData));
            window.location.href = '/result/?type=ziwei';
        } catch (err) {
            console.error(err);
    ...
    const inputBase = [
        ...
        'focus:border-[#6A8FAA] focus:bg-white focus:shadow-[0_0_0_3px_rgba(106,143,170,0.1)]',
    ].join(' ');
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-[2rem] p-6 md:p-10 shadow-xl shadow-sage/5 border border-gray-100 w-full max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="space-y-8">
                <div className="text-center space-y-2">
                    <h2 className="text-2xl md:text-4xl text-foreground font-serif">{t('ziwei.formTitle')}</h2>
                    <p className="text-gray-500 text-[17px] font-sans leading-relaxed">{t('ziwei.formDesc')}</p>
                </div>
                ...
                <div className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex bg-gray-50 p-1 rounded-xl border-2 border-gray-100">
                                <button type="button" onClick={() => setFormData({ ...formData, gender: 'male' })}
                                    className={`${toggleBase} ${formData.gender === 'male' ? 'bg-[#6A8FAA] text-white shadow-sm' : 'text-gray-400'}`}>
                                    {t('common.male')}
                                </button>
                                <button type="button" onClick={() => setFormData({ ...formData, gender: 'female' })}
                                    className={`${toggleBase} ${formData.gender === 'female' ? 'bg-[#6A8FAA] text-white shadow-sm' : 'text-gray-400'}`}>
                                    {t('common.female')}
                                </button>
                            </div>
                        </div>
                    ...
                    <div className="space-y-2">
                        <label className="text-[15px] font-bold text-gray-700 ml-1 block">{t('common.calendar')}</label>
                        <div className="flex bg-gray-50 p-1 rounded-xl border-2 border-gray-100">
                            <button type="button" onClick={() => setFormData({ ...formData, calendarType: 'solar' })}
                                className={`${toggleBase} ${formData.calendarType === 'solar' ? 'bg-sage text-white shadow-sm' : 'text-gray-400'}`}>
                                {t('common.solar')}
                            </button>
                            <button type="button" onClick={() => setFormData({ ...formData, calendarType: 'lunar' })}
                                className={`${toggleBase} ${formData.calendarType === 'lunar' ? 'bg-sage text-white shadow-sm' : 'text-gray-400'}`}>
                                {t('common.lunar')}
                            </button>
                        </div>
                        {formData.calendarType === 'lunar' && (
                            <label className="flex items-center gap-2 mt-2 ml-1 cursor-pointer">
                                ...
                                <span className="text-sm text-gray-500">{t('common.leapMonth')}</span>
                            </label>
                        )}
                    </div>
                ...
                <button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-[#6A8FAA] text-white font-bold rounded-2xl shadow-lg shadow-blue/20 hover:bg-[#5A7E99] transition-all flex items-center justify-center gap-3 disabled:opacity-50 text-[18px] active:scale-[0.98] min-h-[56px] px-6 mt-4"
                >
                    {loading ? (
                        <span className="animate-pulse">{t('ziwei.submitLoading')}</span>
                    ) : (
                        <>
                            <Sparkles className="w-5 h-5 shrink-0" />
                            <span>{t('ziwei.submitBtn')}</span>
                        </>
                    )}
                </button>

                <p className="flex items-center gap-2 justify-center text-xs text-gray-400 font-sans">
                    <Info className="w-3 h-3" /> {t('ziwei.timeNote')}
                </p>
            </div>
        </form>
    );
